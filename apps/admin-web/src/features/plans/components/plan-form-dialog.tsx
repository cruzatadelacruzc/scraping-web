...
export type PlanFormDialogMode = 'create' | 'edit' | 'duplicate';

interface PlanFormDialogProps {
  open: boolean;
  onClose: () => void;
  mode: PlanFormDialogMode;
  /** Required when mode is 'edit' */
  planId?: string | null;
  /** Required when mode is 'duplicate' */
...
/**
 * Dialog for creating, editing, or duplicating a plan.
 *
...
export function PlanFormDialog({
  ...
  planId,
  sourcePlan,
}: PlanFormDialogProps): JSX.Element {
  ...
  const handleConditionToggle = useCallback(
    (condition: string) => {
      const current = watchedFeatures.allowedConditions;
      const updated = current.includes(condition)
        ? current.filter((c) => c !== condition)
        : [...current, condition];
      setValue('features.allowedConditions', updated, { shouldDirty: true });
    },
  ...
  const onSubmit = useCallback(
    (data: PlanFormValues) => {
      if (mode === 'create' || mode === 'duplicate') {
        ...
        updatePlan.mutate({ id: planId, data }, { onSuccess: onClose });
      }
    },
    [mode, planId, createPlan, updatePlan, onClose],
  );
  ...
  const dialogTitle = useMemo(() => {
    switch (mode) {
      case 'create':
        return t('plans.create.title');
      case 'edit':
        return t('plans.edit.title');
      case 'duplicate':
        return t('plans.duplicate.title');
    }
  }, [mode, t]);
  ...
  return (
    <Dialog open={open} title={dialogTitle} onClose={onClose} maxWidth="max-w-xl">
      ...
      >
              ...
              <div className="mt-xs grid grid-cols-2 gap-x-md gap-y-xs">
                {ALLOWED_CONDITIONS.map((condition) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={watchedFeatures.allowedConditions.includes(condition)}
                      onChange={() => {
        ...
        {errors.features && ( // wouldn't normally happen but handle gracefully
