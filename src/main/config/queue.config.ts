...
export class QContext {
   ...
   */
  async QCreate(queueName: string, processor: (job: Job) => Promise<any>): Promise<Queue.Queue> {
    if (!this.queues.has(queueName)) {
      const queue = new Queue(queueName, { redis: process.env.REDIS_URL });      

      queue.on('error', err => { console.error(`Redis connection error for queue "${queueName}":`, err) });
      
      queue.on('ready', () => { console.log(`Queue "${queueName}" is ready.`) });
